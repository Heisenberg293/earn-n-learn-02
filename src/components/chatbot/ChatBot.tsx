...
const ChatBot = () => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => 
        prev.filter(msg => msg.id !== loadingMsgId).concat({
          id: Date.now().toString(),
          content: response,
          sender: "bot",
          timestamp: new Date(),
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {!isOpen && (
        <Button 
          onClick={toggleChat} 
          className="h-14 w-14 rounded-full shadow-lg bg-green-600 hover:bg-green-700 transition-all"
          size="icon"
        >
      ...
      {/* Chat Window */}
      {isOpen && (
        <Card className={`w-full sm:w-[380px] shadow-lg transition-all overflow-hidden ${
          ...
          <CardHeader className="py-3 px-4 flex flex-row items-center justify-between border-b bg-primary text-primary-foreground">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-medium text-sm">SkillSwap Assistant</h3>
            </div>
